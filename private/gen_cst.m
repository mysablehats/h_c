...
for ii = 1:length(b)
    for jj= 1:length(b(ii).mt)
        ...
        try
            ...
            metrics(ii,jj).val = [];
            warning('could not set validation metrics. ignore this warning if they were not set to begin with. ')
        end
        try
            metrics(ii,jj).train = b(ii).mt(jj).confusions.train;
        catch
            metrics(ii,jj).train = [];
            warning('could not set train metrics. ignore this warning if they were not set to begin with. ')
        end

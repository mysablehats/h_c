...
for j = whatIlabel
    if length(kindex)==1
        ...
        if isfield(ssvot,'gas')&&j<=length(ssvot.gas)
            ...
            maybewillslowdownmycodeforonlyabitofspace = eval( ['@' class(ssvot.y)]);
            ssvot.gas(j).class = maybewillslowdownmycodeforonlyabitofspace(full(ind2vec(weirdthingy, size(ssvot.y,1)))); %%% although I liked how it was and it was saving me so much space, confmatrix does not like logicals (it seems to me that it would be the ideal place to use them, but well, i didnt code matlab)
